/*!
 * topolr JavaScript library v1.3.0
 * http://topolr.org/
...
(function () {
    ...
    template.globalMacro = {
        include: function (attrs, render) {
            var p = new template(attrs.template,this._macrofn,this._parameters,this._autodom);
        ...
        self:function (attrs,render) {
            var p = new template(this._source,this._macrofn,this._parameters,this._autodom);
            var t = p.render(attrs.data);
            for (var i in p._caching) {
                this._caching[i] = p._caching[i];
            }
    ...
    template.prototype._macro = function (num, attr) {
        ...
        if (this._macrofn[n.name]) {
            return this._macrofn[n.name].call(this, attr, function () {
                if (n.body !== "") {
                    var inner = new template(n.body,ths._macrofn,ths._parameters,ths._autodom);
    ...
    var autodomc = function (dom, temp, dataarray,macro) {
        this.dom = dom;
        if (is.isString(temp)) {
            this.tempt = topolr.template(temp, null, macro, true);
        } else {
            this.tempt = temp;
        }
    ...
    query.prototype.autodom = function (temp, dataarray,macro) {
        return new autodomc(this, temp, dataarray,macro);
    };
    ...
    module.add({
        ...
        render: function () {
            ...
            try {
                if (ths.autodom) {
                    ths.autodomc = ths.dom.autodom(ths.template, Array.prototype.slice.call(arguments), ths.marcos);
 ...
 */
/**
 * template support recursion <@self data="{{}}"/>
 */